...
extern void messages_afficher_etat( const etat_coureur_t etat ) {
...
static void messages_afficher_type( long type ) {
...
static void messages_afficher_corps( corps_requete_t * corps ) {

	printf("\tDossard = %d\n" , corps->dossard ); 
	printf("\tEtat    = " );   
	messages_afficher_etat( corps->etat );  
	printf("\n");
}
 ...
 */
extern void messages_afficher_requete( requete_t * const requete ) {
  
	/* Type du message */ 
	printf("\tType = ") ; messages_afficher_type(requete->type) ; printf("\n"); 

	/* Corps */
	messages_afficher_corps( &(requete->corps) ); 
	printf("\n");
}
 ...
 */
extern void messages_afficher_reponse( reponse_t * const reponse ) {
	...
	if( reponse->corps.nb_coureurs > 0 ) {
		...
		for( i=0 ; i < reponse->corps.nb_coureurs ; i++)
			printf( "\t|  %5d  |      %3d |\n" ,reponse->corps.tab_coureurs[i].dossard , reponse->corps.tab_coureurs[i].distance );
		/* -- Pied classement */
		printf("\t*---------*----------*\n\n" );
	}
}
 ...
 */
extern void messages_afficher_parcours( reponse_t * const reponse ) {
 ...
 */
extern void messages_initialiser_attente() {
  srandom(getpid()) ;
}
 ...
 */
extern void messages_attendre_tour() {
  sleep(random()%10) ;
}

extern void messages_afficher_erreur( err_t cr ) {
